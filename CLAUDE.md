...
- **The shared instance's short domain.** No hostname is assigned to the `kurze-url-api` project other than the API's own, so `SHARED_DOMAIN_HOSTNAME` is `short.invalid` in Production and Preview — a name RFC 2606 reserves as permanently unresolvable, chosen so nothing appears to work while the real one is undecided. Short links do not function until this is settled. Two constraints on whatever replaces it: it must route to **`kurze-url-api`** (`kurze-url-web.vercel.app` serves the frontend, whose router has no redirect surface — a slug there hits the React 404 page and the API is never consulted), and it must differ from `API_HOSTNAME`, or the API surface answers first and `GET /{slug}` is unreachable. Changing the value upserts a _new_ `domain` row rather than renaming the old one, so each attempt leaves the previous hostname behind as a `team_id IS NULL` row — the ones from `localhost`, `kurze-url-web.vercel.app` and `kurze-url.vercel.app` were cleared on 2026-09-05, and settling on the real domain will leave `short.invalid` to clear the same way. Delete only rows with no links: `link.domain_id` is `on delete cascade`, so dropping a domain drops every link on it.
