...
- `PUT /v1/links/{link_id}/password` — set or change the password (body: new plaintext password; server hashes with Argon2id per `05-database-schema.md`). Kept as its own endpoint, separate from the general `PATCH`, so it maps to its own distinct, auditable `audit_log` action — `link.password_set` on the first call, `link.password_changed` on every one after, decided by a read inside the write's own transaction rather than before it, so two near-simultaneous requests can't both observe "no password yet" and both log `password_set` — and so the tighter rate limit on password-related mutations (per `05-database-schema.md`) can be scoped to exactly this route. Answers `200` with the full `Link` (same shape as `GET`/`PATCH`, `has_password: true`). A password the policy (`apps/api/internal/auth/policy.go`) rejects answers `422` with a typed `ErrorDetail` — `Location: "body.password"`, `Value` the policy's own reason token — the same pattern `POST /v1/teams`'s slug conflict uses above, keyed by a body field instead of a path parameter, so the frontend keys its message off the token rather than parsing free text.
- `DELETE /v1/links/{link_id}/password` — remove password protection; logs `link.password_removed`. Answers `200` with the full `Link` (`has_password: false`) and is **idempotent**: called again on a link that already has no password, it still answers `200` unchanged rather than `404` — a 404 there would tell the caller nothing it doesn't already know, while forcing every client to special-case it.
