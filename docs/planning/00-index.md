...
- **Rate-limit values were settled on 2026-09-08 by deciding what the limiters are for**, rather than by picking numbers. Each one caps a single subject inside a single window, which stops one runaway client and cannot bound a shared free-tier quota — so the redirect limit is set for what NAT'd traffic needs and explicitly does not defend Upstash's monthly command budget, which moves to monitoring. The one quota that could not be left to monitoring got a second, instance-wide limit: invitations are capped globally over a rolling 30 days, because sign-in is a magic link with no password fallback and running Resend's quota dry would lock every Verein out at once. Per-limit reasoning lives in `apps/api/.env.example`.
