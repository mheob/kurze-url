...
| UI components | **shadcn/ui on Base UI** (decided 2026-09-01, reversed 2026-09-13) | See "Primitive layer: Radix, then Base UI" below — the exception made for Tremor compatibility no longer applies once Tremor was dropped |
| Analytics dashboard | **shadcn `chart` on Recharts v3** | Replaces Tremor (dropped 2026-09-13). No pre-built dashboard blocks, so KPI tiles, time series and breakdowns become ordinary composition work — against a component in the same design system rather than a second vendor with its own theming. See "Primitive layer: Radix, then Base UI" below. |
...
| Fonts | **Geist and Geist Mono, self-hosted** (decided 2026-09-13) | Loaded from `@fontsource-variable/geist` and `@fontsource-variable/geist-mono`, not a font CDN — a privacy constraint, not a styling preference. A CDN request hands the visitor's IP address to a third party on every page load, and the audience is German associations and their visitors; self-hosting removes that transfer instead of explaining it away in the privacy policy `08-legal-and-compliance.md` already owes them. Named by the `base-sera` preset, which specifies both families. |
...
## Primitive layer: Radix, then Base UI
...
**Reversed 2026-09-13.** The condition above was met — not by Tremor migrating to Base UI, but by the second branch: the analytics dashboard is being built on shadcn's own `chart` component instead of Tremor, and `chart` is bare Recharts v3 with no primitive dependency of its own (it ships in Base UI, React Aria and Radix variants alike, per `docs/superpowers/specs/2026-09-13-design-system-and-shell-design.md`). The two-libraries argument that drove the original decision cannot survive that: with no dashboard vendor left carrying its own primitives, there is nothing left for Base UI to conflict with.

The measurements that made the reversal cheap were taken before touching a line of code, not assumed. **Tremor was never installed** — it appears in no `package.json` anywhere in this repository, at any point. **Radix's entire footprint in `apps/web` had shrunk to one import** — `Slot`, in `src/components/ui/button.tsx`, the only shadcn component the app had generated by 2026-09-13. Neither fact was true when the original decision above was made on 2026-09-01, when Tremor was still the committed analytics choice; both were true by the time the design spec reopened the question, which is exactly the reopening the paragraph above asked for.

The reversal itself ran `pnpm dlx shadcn@latest init --preset b39ODpImW -b base`, style `base-sera` (see `CLAUDE.md`'s Stack table) — `-b base` is not optional, since `--preset` alone does not carry the primitive layer and the CLI defaults to `radix`. `apps/web/package.json` no longer lists any `@radix-ui/*` package, and `ui/button.tsx` now imports `Button` from `@base-ui/react/button` — verified by grep against the repository, not assumed. Analytics itself is still unbuilt — this change did not render a chart — but it now inherits `chart` as a component in the same design system, not a second vendor's dependency to install.
