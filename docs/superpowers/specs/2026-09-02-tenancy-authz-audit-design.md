...
An operation-coverage guard (`TestEveryOperationIsAccountedFor` / `checkOperationCoverage` in `internal/api`) walks every operation Huma has registered — read off the generated OpenAPI document — and requires each one to be accounted for: an operation declaring `bearerAuth` must either appear in the permission matrix or be named in an explicit `notTeamScoped` allowlist with a reason (`GET /v1/me`, `POST /v1/teams`, `GET /v1/teams`); an operation declaring no `bearerAuth` at all must be named in a separate `publicOperations` allowlist with a comment saying why it is genuinely public. What shipped keys this off operation-ID strings in those two hand-maintained maps, not off reflecting on whether the operation's input type actually embeds one of the four scopes. That is behaviorally stronger in one respect — it also catches an operation that forgot `Security: bearerAuth` entirely, which a type-level check alone would miss — but it leaves a gap a type-level check would have closed: a team-scoped operation mistakenly added to `notTeamScoped` passes the guard even though nothing then verifies it embeds a scope.
...
- **`internal/api`** — handler tests through `humatest`. The load-bearing one is a table-driven permission matrix: every endpoint crossed with every role, asserting the expected status, so the matrix in this document is executable rather than prose. Plus the operation-coverage guard asserting every registered operation is accounted for — a `bearerAuth` operation via the permission matrix or a named `notTeamScoped` exemption, an unauthenticated one via a named `publicOperations` entry (see "The authorization mechanism" above for what this does and does not prove), and a test asserting `/openapi.json` names the generic envelope schemas (`PageTeam`, `PageMember`, `PageAuditEntry`) readably.

## Definition of done

- `cd apps/api && go vet ./... && go test ./... && golangci-lint run` is clean.
- The permission matrix test passes for every endpoint and every role.
- The operation-coverage guard fails if a new operation is registered with neither a permission-matrix row (or a named `notTeamScoped` exemption) nor a `publicOperations` entry. It does not, by itself, prove the operation's input type embeds a scope — see the residual gap noted under "The authorization mechanism" above.
